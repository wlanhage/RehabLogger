...
import { chatComplete, type ChatTurn } from "@/lib/ai/gemini";
import {
  COACH_SYSTEM_PROMPT,
...
The conversation that follows is the user's chat with you. Keep replies short and actionable — usually 3–8 sentences or a tight bullet list.`;
  ...
  const reply = await chatComplete(messages, {
    temperature: 0.5,
    maxTokens: 1500,
    // Light thinking budget — enough to reason about symptom trends but not
    // blow the output cap. Plain coaching replies don't need extended reasoning.
    thinkingBudget: 256,
  });
